...
void tcp_server(void *pvParam){
    ...
    unsigned long accum_readbyte = 0;
    double speed;
    struct timeval t;
    //int cs;//client socket
    //int global_cnt = 0;
    uint64_t app_start_us = esp_timer_get_time();
    //int rs = 0;
    ...
    while(1){
        ...
        ESP_LOGI(TAG, "... allocated socket\n");
         if(bind(sc, (struct sockaddr *)&tcpServerAddr, sizeof(tcpServerAddr)) != 0) {
        ...
        while(1){
            /**rs = write(sc,data_send,1000); // 1kbyte sending.
            if(rs <0){
                 printf("Send retv < 0");
              }
              else if(rs == 0){
            ...
            clientConn = accept(sc,(struct sockaddr *)&remote_addr, &socklen);
            t.tv_sec = 10; // Timeout
            setsockopt(clientConn, SOL_SOCKET, SO_RCVTIMEO, &t, sizeof(t));
            ESP_LOGI(TAG,"New connection request!");
            while (1) {
                 //bzero(data_read, sizeof(data_read));
                //ssize_t sizeRead = read(clientConn, data_read, sizeof(data_read));
                 ssize_t sizeRead = recv(clientConn, data_read, sizeof(data_read), 0);
               if(sizeRead < 0)
               {
                printf("error recv: %d %s", sizeRead, strerror(errno));
               }
               else if (sizeRead == sizeof(data_read))
               {
                accum_readbyte += sizeRead;
                //printf("%d", accum_readbyte);
                ...
                //}
                   ...
                   if (esp_timer_get_time() - app_start_us > 5000000) {

                        //printf("Time: %llu", esp_timer_get_time());
                        speed =  ((double) (accum_readbyte / ((esp_timer_get_time() - app_start_us) / 1000000))) / 1000000.0;
                        dataSum = 0;
